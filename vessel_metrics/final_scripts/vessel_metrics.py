...
def overlay_segmentation(
    ...
    im_cmap="gray",
    label_cmap="jet",
    save_path=None,  # new optional argument to save figure
):
    if contrast_adjust:
    ...
    if save_path:
        ...
        try:
            ...
            if not hasattr(main, '__file__'):
                # likely interactive environment like IPython/Jupyter
                plt.show(block=False)
            else:
                # Non-interactive (script), just close to avoid errors
                plt.close()
        except ImportError:
            plt.close()
...
def preprocess_czi(input_directory, file_name, channel=0):
    ...
    # Load image metadata (no full image in RAM yet)
    img = AICSImage(
        input_directory + file_name,
        reader=aicsimageio.readers.CziReader
    )
    ...
    # Get image shape in ZYX order for the requested channel
    data_shape = img.get_image_dask_data("ZYX", T=0, C=channel).shape
    z, y, x = data_shape

    # Separate crop sizes for Y and X
    crop_y = 3000
    crop_x = 2000
    downsample_factor = 2
    ...
    yc, xc = y // 2, x // 2
    y1, y2 = yc - crop_y // 2, yc + crop_y // 2
    x1, x2 = xc - crop_x // 2, xc + crop_x // 2

    print(f"[DEBUG] Original dataset shape (ZYX): {data_shape}")

    # Get only the cropped region from disk as Dask array
    cropped_dask = img.get_image_dask_data("ZYX", T=0, C=channel)[:, y1:y2, x1:x2]

    print(f"[DEBUG] Cropped region shape: {cropped_dask.shape}")

    # Downsample in Y and X before loading into memory
    cropped_dask = cropped_dask[:, ::downsample_factor, ::downsample_factor]

    print(f"[DEBUG] Downsampled region shape (pre-compute): {cropped_dask.shape}")

    # Load into memory now as float32
    image = cropped_dask.compute().astype(np.float32)
...
def fix_skel_artefacts(skel):
    ...
    bp_labels = bp_labels.astype(np.uint16)

    new_edge_num = edge_count + 1

    for i in np.unique(bp_labels)[1:]:  # Skip background
        connected_segs = find_connected_segments(bp_labels, edge_labels, i)
        if len(connected_segs) == 2:
            coord_list = []
            bp_conns = np.zeros_like(edges, dtype=np.uint16)
            for c in connected_segs:
                ...
                if np.sum(temp_seg == 1):
                    ...
                    coords, _ = find_endpoints(temp_seg)
...
def find_connected_segments(bp_labels, edge_labels, bp_num):
    ...
    # Create a signed integer array to hold -1 values
    temp_bp = np.zeros_like(bp_labels, dtype=np.int32)
    
    for i in this_bp_inds:
        temp_bp[i[0], i[1]] = -1

    # Convert edge_labels to int32 so that addition is safe with temp_bp
    bp_el = edge_labels.astype(np.int32) + temp_bp
...
def parameter_analysis(im, seg, params, output_path, file_name):
    ...
    if "diameter" in params:
        ...
        # Extract only the diameter values (second item in each tuple)
        diam_values = [d[1] for d in diameters]
...
def select_roi(image, output_dir, slice_number=""):
    try:
        ...
        raise ImportError("easygui is required for GUI functions. Please install it if you want to use the GUI.")
    recrop = True
    while recrop == True:
